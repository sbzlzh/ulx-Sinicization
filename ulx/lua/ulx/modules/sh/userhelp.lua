...
无法访问.如果用户在其用户允许列表或允许列表中有"ulx slap"
...
让每个人都可以使用"ulx teleport"命令,但只允许他们传送自己.

下面在 userallow 和 groupallow 命令中给出了使用访问标签的示例.格式
访问标签如下.传递给命令的每个参数都可以由
访问标签.每个被限制的参数必须按照与命令中相同的顺序列出,
用空格隔开. 如果您不想限制参数,请使用星号 ("*").EG,限制"ulx slap"伤害从 0 到 10,但仍然允许它用于任何人,使用标签"* 0:10".

用户管理命令:
ulx adduser <user> <group> - 将指定的 CONNECTED 播放器添加到指定的组.
该组必须存在才能使此命令成功.使用操作员、管理员、超级管理员或查看 ulx
添加组.您只能指定一组.关于免疫的解释见上文.
例 1. ulx adduser "Someguy"超级管理员 -- 这会将连接的 "Someguy"添加为超级管理员
Ex 2. ulx adduser "Dood"monkey -- 这会将连接的 "Dood"添加到组猴子
  在组存在的条件下
...
ulx removegroup <group> - 永久删除组.还从所有连接的组中删除组
  用户和所有将来连接的用户.如果用户除此之外没有任何组,他们将
  成为客人.请务必小心使用此命令！

ulx renamegroup <当前组> <新组> - 重命名组

ulx setgroupcantarget <group> [<target string>] - 限制组可以定位的用户.通过没有
  清除限制的论据.
