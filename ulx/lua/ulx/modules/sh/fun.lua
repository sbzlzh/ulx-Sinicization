...
cloak:addParam{ type=ULib.cmds.NumArg, min=0, max=255, default=255, hint="数量", ULib.cmds.round, ULib.cmds.optional }
...
blind:addParam{ type=ULib.cmds.NumArg, min=0, max=255, default=255, hint="数量", ULib.cmds.round, ULib.cmds.optional }
	...
	for i=1, #target_plys do
		...
		if not should_unragdoll then
			if ulx.getExclusive( v, calling_ply ) then
				ULib.tsayError( calling_ply, ulx.getExclusive( v, calling_ply ), true )
			elseif not v:Alive() then
				ULib.tsayError( calling_ply, v:Nick() .. " 已经死了,不能被人玩弄!", true )
			else
				ragdollPlayer( v )
				table.insert( affected_plys, v )
			end
...
function ulx.maul( calling_ply, target_plys )
	...
	ulx.fancyLogAdmin( calling_ply, "#A 殴打#T", affected_plys )
end
