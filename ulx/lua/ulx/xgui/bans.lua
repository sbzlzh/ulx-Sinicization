...
local sortMode = 0
local sortAsc = false
xbans.sortbox = xlib.makecombobox{ x=185, y=6, w=150, text="排序:日期(降序)", choices={ "日期","名称","Steam ID","管理员","原因","解禁日期","禁用长度" }, parent=xbans }
function xbans.sortbox:OnSelect( i, v )
	...
	self:SetValue( "种类: " .. v .. (sortAsc and " (Asc.)" or " (Desc.)") )
	xbans.setPage( 1 )
	xbans.retrieveBans()
end
...
xbans.btnFreezeBan = xlib.makebutton{ x=80, y=340, w=95, label="解除封禁...", parent=xbans }
xbans.btnFreezeBan.DoClick = function() banUserList( true ) end

xbans.infoLabel = xlib.makelabel{ x=204, y=344, label="右键单击禁令以获取更多选项", parent=xbans }


xbans.resultCount = xlib.makelabel{ y=344, parent=xbans }
function xbans.setResultCount( count )
	local pnl = xbans.resultCount
	pnl:SetText( count .. " 结果" )
	pnl:SizeToContents()
...
function xbans.ShowBanDetailsWindow( bandata )
	...
	xbans.openWindows[bandata.steamID] = xlib.makeframe{ label="封禁详情", x=wx, y=wy, w=285, h=295, skin=xgui.settings.skin }
	...
	xlib.makelabel{ x=90, y=70, label=bandata.time and ( os.date( "%b %d, %Y - %I:%M:%S %p", tonumber( bandata.time ) ) ) or "<此封禁没有原数据>", parent=panel }
	...
	xlib.makelabel{ x=10, y=110, label="封禁长度:", parent=panel }
	...
	xlib.makelabel{ x=33, y=130, label="剩余时间:", parent=panel }
	...
	xlib.makelabel{ x=26, y=150, label="被封禁:", parent=panel }
	...
	xlib.makelabel{ x=13, y=205, label="最近更新时间:", parent=panel }
	xlib.makelabel{ x=90, y=205, label=( ( bandata.modified_time == nil ) and "从不" or os.date( "%b %d, %Y - %I:%M:%S %p", tonumber( bandata.modified_time ) ) ), parent=panel }
...
function xgui.ShowBanWindow( ply, ID, doFreeze, isUpdate, bandata )
	...
	xlib.makelabel{ x=10, y=108, label="封禁长度:", parent=xgui_banwindow }
