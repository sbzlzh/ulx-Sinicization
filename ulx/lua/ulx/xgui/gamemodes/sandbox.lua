...
xlib.makecheckbox{ x=10, y=150, label="骨操纵.杂项", repconvar="rep_sbox_bonemanip_misc", parent=sbox_settings }
xlib.makecheckbox{ x=10, y=170, label="骨操纵.npc", repconvar="rep_sbox_bonemanip_npc", parent=sbox_settings }
xlib.makecheckbox{ x=10, y=190, label="骨操纵.玩家", repconvar="rep_sbox_bonemanip_player", parent=sbox_settings }

xlib.makelabel{ x=5, y=247, w=140, wordwrap=true, label="注意:在 XGUI 中可配置的非 ulx cvar 仅用于方便访问,并且在服务器关闭或崩溃时不要保存.", parent=sbox_settings }
	...
	for g, limits in ipairs( xgui.data.sboxlimits ) do
		if #limits > 0 then
			...
			for _, cvar in ipairs( limits ) do
				...
				xgui.queueFunctionCall( xlib.makeslider, "sboxlimits", { x=10+(i%2*205), y=5+math.floor(i/2)*25, w=200, label="最大限度 " .. cvardata[1]:sub(9), min=0, max=cvardata[2], repconvar="rep_"..cvardata[1], parent=panel, fixclip=true } )
				i = i + 1
			end
			sbox_settings.plist:Add( xlib.makecat{ label=limits.title .. " (" .. #limits .. " 限制" .. ((#limits > 1) and "" or "") .. ")", contents=panel, expanded=( g==1 ) } )
		end
	end
end
