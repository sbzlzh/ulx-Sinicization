...
if not xgui.settings.moduleOrder then xgui.settings.moduleOrder = { "命令", "组", "地图", "设置", "封禁" } end
if not xgui.settings.settingOrder then xgui.settings.settingOrder = { "沙盒", "服务器", "客服端" } end
...
function xgui.init( ply )
	...
	local infoLabel = string.format( "\nULX 管理员模组 :: XGUI - Team Ulysses |  ULX %s  |  ULib %s 汉化 by 随波逐流", ULib.pluginVersionStr("ULX"), ULib.pluginVersionStr("ULib") )
