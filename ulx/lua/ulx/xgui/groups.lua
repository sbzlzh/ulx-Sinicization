...
groups.cplayer.DoClick = function()
	if groups.players:GetSelectedLine() then
		...
		menu:AddSpacer()
		menu:AddOption( "删除用户", function() groups.changeUserGroup( ID, "user" ) end )
		menu:Open()
	end
end
...
groups.teambutton = xlib.makebutton{ x=5, y=275, w=160, label="管理团队 >>", disabled=not ulx.uteamEnabled(), parent=groups.pnlG1 }
...
function groups.addBySteamID( group )
	...
	xlib.maketextbox{ x=5, y=30, w=180, parent=frame, selectall=true, text="进入STEAMID..." }.OnEnter = function( self )
		if ULib.isValidSteamID( self:GetValue() ) then
			...
			Derma_Message( "SteamID 无效!", "XGUI NOTICE" )
		end
	end
end
...
groups.newgroup = xlib.makebutton{ x=245, y=175, w=100, label="创建新的...", parent=groups.pnlG2 }
groups.newgroup.DoClick = function()
	if not ULib.ucl.groups[groups.gname:GetValue()] then
		...
		Derma_Message( "同名群组已存在!", "XGUI 注意" );
	end
end
xlib.makelabel{ x=145, y=8, label="姓名:", parent=groups.pnlG2 }
xlib.makelabel{ x=145, y=33, label="继承自:", parent=groups.pnlG2 }
xlib.makelabel{ x=145, y=58, label="可以瞄准:", parent=groups.pnlG2 }
groups.gname = xlib.maketextbox{ x=180, y=5, w=165, text="新组", selectall=true, parent=groups.pnlG2 }
groups.ginherit = xlib.makecombobox{ x=215, y=30, w=130, text="用户", parent=groups.pnlG2 }
...
groups.gupdate.DoClick = function( self )
	...
	if groups.gname:GetValue() ~= groupname then
		if groupname == "superadmin" or groupname == "admin" then
			Derma_Query( "重命名 " .. groupname .. " group 通常是一个坏主意,它可能会破坏一些插件.你确定吗?", "XGUI 警告",
...
groups.gdelete.DoClick = function()
	...
	if group == "superadmin" or group == "admin" then
		Derma_Query( "删除 " .. group .. " group 通常是一个坏主意,它可能会破坏一些插件.你确定吗?", "XGUI 警告",
			"Remove", function()
				RunConsoleCommand( "ulx", "removegroup", group ) end,
			"Cancel", function() end )
	else
		Derma_Query( "您确定要删除 \"" .. group .. "\" group?", "XGUI 警告",
			"Remove", function()
				RunConsoleCommand( "ulx", "removegroup", group ) end,
			"Cancel", function() end )
	end
...
xlib.makebutton{ x=5, y=160, w=80, label="创建新的", parent=groups.pnlG3 }.DoClick = function()
...
function groups.pnlG4:Open()
	if groups.pnlG3:IsVisible() then
		groups.teambutton:SetText( "管理团队 >>" )
		groups.pnlG3:Close()
	end
	self:openAnim()
...
xlib.makelabel{ x=5, y=5, label="可以访问:", parent=groups.pnlG4 }
		...
		if not arg.type.invisible then
			...
			if not arg.invisible then
				...
				if arg.type == ULib.cmds.PlayerArg or arg.type == ULib.cmds.PlayersArg then
					...
					outPanel.type = "ply"
					outPanel.cantarget = xlib.makecheckbox{ x=5, y=5, label="忽略 can_target", value=ignoreCanTarget or 0, parent=outPanel }
				...
				elseif arg.type == ULib.cmds.NumArg then
					...
					if table.HasValue( arg, ULib.cmds.allowTimeString ) then
						...
						xlib.makelabel{ x=25, y=8, label="限制最小值", parent=outPanel }
						xlib.makelabel{ x=25, y=48, label="限制最大值", parent=outPanel }
				...
				elseif arg.type == ULib.cmds.StringArg then
					...
					outPanel.list:AddColumn( "白名单字符串值" )
						...
						if self:GetValue() ~= "" then
							if not( string.find( self:GetValue(), "<" ) or string.find( self:GetValue(), ">" ) or string.find( self:GetValue(), "," ) ) then
								...
								if not found then
									...
									Derma_Message( "此项目已存在于列表中!", "XGUI NOTICE" )
								end
							else
								Derma_Message( "不能使用包含以下字符的字符串: < > ,", "XGUI NOTICE" )
							end
						end
					end
